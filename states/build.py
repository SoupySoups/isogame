...
class build(state):
    ...
    def onStart(self):
        ...
        self.saveButton = self.uiContainer.add(gui.Button(0.1, 0.7, 0.8, 0.2, leftClickCallback=self.save))
    ...
    def onUpdate(self, events):
        ...
        for e in events:
            if e.type == pygame.MOUSEBUTTONDOWN:
                if self.height < len(self.lm.activeLevel.layers)-1 and e.button == 4: self.height += 1
                if self.height > 0 and e.button == 5: self.height -= 1
            if e.type == pygame.KEYDOWN:
                if e.key in [pygame.K_1, pygame.K_2, pygame.K_3, pygame.K_4, pygame.K_5, pygame.K_6, pygame.K_7, pygame.K_8, pygame.K_9, pygame.K_0]:
                    self.activeBlock = int(e.unicode)
        
        # Get the mouse position
        x, y, z = self.rm.snapPointToGrid(self.rm.screenToWorld(self.rm.removeCameraOffset(self.gm.windowToScreen(pygame.mouse.get_pos())), self.height))
        l = self.lm.activeLevel.getLayer(self.height)

        xyLimited = self.rm.limitPoint(l.width, l.height, (x, y))
        
        pressed = pygame.mouse.get_pressed()
        if not self.uiContainer.hovered: # If the mouse is interacting with the UI, don't do anything to the level
            if pressed[0]:
        ...
        self.em.animate()
        self.rm.render(self.lm.activeLevel, self.gm.screen, self.height, True, )
